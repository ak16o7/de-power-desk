...
class HttpSmokeTests(unittest.TestCase):
    ...
    def test_health_html_and_static_assets(self):
        self.assertEqual(self.client.get('/health').json()['version'],m.VERSION)
        html=self.client.get('/').text
        self.assertIn('DE Power Desk',html);self.assertNotIn('__REFRESH_SECONDS__',html)
        for asset in ('style.css','plotly.min.js','app.js'):
