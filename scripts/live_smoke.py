...
def main():
    ...
    for neighbor in m.DEFAULT_NEIGHBORS:
        for doc,label,contract in [('A11','physical',None),('A09','scheduled','A01'),('A09','total','A05')]:
            extra={'contract_MarketAgreement.Type':contract} if contract else {}
            try:
                incoming=flat(raw(doc,in_Domain=m.AREAS['DE_LU'],out_Domain=m.AREAS[neighbor],**extra))
                outgoing=flat(raw(doc,in_Domain=m.AREAS[neighbor],out_Domain=m.AREAS['DE_LU'],**extra))
            except StopIteration:
                checks.append({'name':neighbor+' '+label,'skipped':'no upstream response'});continue
    ...
    print(json.dumps({'version':m.app.version,'comparisons':len(checks),'points':sum(c['points'] for c in checks),'outages':results['outages']['data']['zones'],'activation':results['balancing']['data']['sources']['12.3.E']},ensure_ascii=True))
