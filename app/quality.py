...
def panel_quality(panel, data):
    ...
    if panel=='outages':
        ...
        activation_ok=src.get('NTP',{}).get('state')=='ok' or src.get('12.3.E',{}).get('state')=='ok'
        complete=activation_ok and all(src.get(k,{}).get('state')=='ok' for k in ('A85','A86'))
        complete=complete and (present('Net activation') or present('aFRR net DE')) and present('Net imbalance volume') and any(present(k) for k in ('Imbalance price','Imbalance price long','Imbalance price short'))
